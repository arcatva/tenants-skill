...
- **Must run as non-root** — the platform runs containers as UID 65532 with a hardened security profile (no privilege escalation, all Linux capabilities dropped, seccomp `RuntimeDefault`). End the Dockerfile with `USER 65532` and listen on a high port (8080). A root image is rejected at deploy. See the 🔒 note and per-language templates in [app-templates.md](app-templates.md).
...
# expected: a non-empty, non-zero numeric UID, e.g. USER=[65532]
# USER=[] or USER=[root] or USER=[0] → will be rejected; add `USER 65532` to the Dockerfile
```
